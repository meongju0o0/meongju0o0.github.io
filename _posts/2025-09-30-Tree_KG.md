...
## Methodology
### 트리 형태의 계층적 그래프 (Tree-like Hierarchical Graph)
- **Tree-KG**가 생성하는 지식 그래프의 기본 구조
- 마치 회사의 조직도처럼 여러 계층(layer)으로 구성

- 그래프 ($G=(V,E)$)
- 정점 (Vertex, $V$)
    - 그래프의 각 개념(엔티티)은 특정 계층($V_1, V_2, ..., V_k$)에 속함
- 간선 (Edge, $E$)
    - 수직 간선 (Vertical Edges, $E_1$)
        - 서로 다른 계층의 정점 간에 연결 (e.g., '물리학' 서적 → '전자 기학' 단원)
    - 수평 간선 (Horizontal Edges, $E_2$)
        - 동일한 계층 내의 정점 간에 연결 (e.g., '전하' 개념 ↔ '전자' 개념)

- 한편, 계층 간 정점 집합은 **상호배타적**이며 **총체적망라**임
    - 상호배타적: $V_1 \cap V_2 \cap ... \cap V_k = \emptyset$
    - 총체적망라: $V_1 \cup V_2 \cup ... \cup V_k = U$

- 수직 간선과 수평 간선은 **총체적망라**임
    - $E = E_1 \cup E_2$

![mece](/images/2025-09-30-Tree_KG/mece.png)

### 지식 그래프 스키마 (KG Schema)
- 지식 그래프를 구성하는 정점과 간선의 구체적인 설계 명세
- 정점의 속성
    - 이름 (name)
        - 정점의 고유한 이름 (e.g., "Electric Charge")
    - 설명 (description)
        - 텍스트에서 추출한 개념 설명
        - 주변 정점의 정보를 반영해 더 풍부한 의미 정보를 담아냄
    - 관계 (relations)
        - 해당 정점에 연결된 간선 정보
- 간선의 종류
    - 수직 간선 (Vertical Edge)
...
### 아다믹-아다 점수 (Adamic-Adar Score)
### 공통 조상 노드 수 (Number of Common Ancestors)