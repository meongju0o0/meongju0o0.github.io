...
- **정답(Ground Truth)이 있는 경우**
    ...
    - **(a) Entity Recall (ER, 재현율)**
        ...
        4. 위 과정을 $G_{gt}$의 모든 엔티티에 대해 반복한 뒤 ER을 계산

        $$ER=\frac{\text{# mapped entities}}{\vert V_{gt} \vert}$$
    ...
    - **(b) Precision (PC, 정밀도)**
        ...
        4. 위 과정을 $G$의 모든 엔티티에 대해 반복한 뒤 PC를 계산

        $$PC=\frac{\text{# mapped entities}}{\vert V \vert}$$

    - **(c) F1 Score (F1)**
        - ER과 PC의 조화평균

        $$F1=2\times\frac{\text{ER}\times\text{PC}}{\text{ER}+\text{PC}}$$
    ...
    - **(d) Mapping-based Edge Connectivity (MEC, 연결성)**
        ...
        $$MEC=\frac{\text{# mapped entity pairs connected in G}}{\vert E_{gt} \vert}$$
    ...
    - **(e) Mapping-based Edge Distance (MED, 거리)**
        ...
        $$MED=\frac{1}{\vert E \vert_{gt}}\sum_{(u,v) \in E_{gt}} \frac{d_G(u,v)}{\overline{d_G}}$$
...
- **정답(Ground Truth)이 없는 경우**
    ...
    - **(f) Entity Specificity (ES, 엔티티 전문성)**
        ...
        $$ES=\frac{\sum \text{specificity}}{\vert V \vert}$$
    ...
    - **(g) Entity Completeness (EC, 엔티티 완성도)**
        ...
        $$EC=\frac{\sum \text{completeness}}{\vert V \vert}$$

    - **(h) Relation Strength (RS, 관계 강도)**
        - 두 엔티티 간의 관계(간선)가 얼마나 가깝고 명료한가?
        - LLMs를 활용하여 0점에서 10점 사이로 평가

        $$RS=\frac{\sum \text{strength}}{\vert E \vert}$$
    
    - **Entity Scoring Example**

    ```text
    - Role: You are an educational knowledge graph expert skilled in evaluating entity extraction quality.
    - Task: Given a knowledge domain, chapter overview, and extracted entities, score each entity's specificity and description completeness.
    - Constraints
...
### 4.3 Token/API Cost
<div align="center">
    <img src="/images/2025-09-30-Tree_KG/token_api_cost.png" alt="token_api_cost" width="600">
</div>
...
#### 4.4.1 Summerization
<div align="center">
    <img src="/images/2025-09-30-Tree_KG/w_wo_summary.png" alt="w_wo_summary" width="600">
</div>
...
<div align="center">
    <img src="/images/2025-09-30-Tree_KG/multiple_contextual_based_convolution.png" alt="multiple_contextual_based_convolution" width="600">
</div>

<div align="center">
    <img src="/images/2025-09-30-Tree_KG/convolution_graph_comparison.png" alt="convolution_graph_comparison" width="800">
</div>
...
<div align="center">
    <img src="/images/2025-09-30-Tree_KG/entity_deduplication_threshold.png" alt="entity_deduplication_threshold" width="500">
</div>

- 엔티티 중복 제거(dedup) 과정에서 가장 효율적인 임계값($threshold_{dedup}$)
    1. 임계값 증가에 따른 경향
        - 임계값이 높아질수록 LLM이 확인해야 하는 전체 확인 횟수와 실제로 병합에 성공한 중복 제거 횟수가 모두 증가
    2. 비효율 구간(0.55 $0.55 \rightarrow 0.6$)
        - 성공 횟수: **7%** 증가
        - 체크 횟수: **124%** 폭증

#### 4.4.4 Edge Prediction
<div align="center">
    <img src="/images/2025-09-30-Tree_KG/mean_percentile_by_weight_combination.png" alt="mean_percentile_by_weight_combination" width="600">
</div>
...
- 가중치의 의미와 설정 원칙
    ...
    - 연구진은 코사인 유사도가 가장 중요한 단서를 제공하고 그 다음으로 AA, 마지막으로 CA라 판단
